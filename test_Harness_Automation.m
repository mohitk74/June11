...
% Creating the test harness having source as 'From Workspace' and Sink 'Outport'
  % sltest.harness.create(modelName, 'Name', 'and_gate_model_Harness', 'Source', 'From Workspace', 'Sink', 'Outport');
% Opening the test Harness model
% sltest.harness.open(modelName,'and_gate_model_Harness');
%--------------------------------------------------------------------------
% test Manager
tf = sltest.testmanager.TestFile('and_gate_model_Results12.mldatx');
ts = getTestSuites(tf);
tc = getTestCases(ts);
% To provide the Sample Model & Test Harness Model to the Test Manager
setProperty(tc,'Model',modelName)
% setProperty(tc, 'HarnessName', 'and_gate_model_Harness', 'HarnessOwner', modelName);
...
exampleFile = 'and_gate_model_Results12.mldatx';
